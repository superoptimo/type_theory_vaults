...
**What breaks without this:** without a finitary, syntactic, checkable notion of proof, everything the rest of the book wants to say — that logical implication is a *tractable* relationship, not just a true-by-definition one — is unstatable. The [[Soundness-and-Completeness|soundness and completeness]] theorems of Section 2.5 are precisely the bridge between this section's syntactic $\vdash$ and Section 2.2's semantic $\models$; without $\vdash$ existing as a well-defined object first, there's nothing for that bridge to connect.
...
From $\alpha$ and $\alpha \rightarrow \beta$, infer $\beta$. That's the *only* rule — everything else the calculus can do has to be encoded into [[Weak-Fragments-of-Number-Theory#The axioms|the axioms]].
