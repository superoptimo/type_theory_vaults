...
This is worth pausing on precisely because it is not a novel idea in this section — it's the induction principle from Section 1.4, instantiated with "depth of formation tree" as the recursively-defined quantity, restated in circuit language ("boxes a signal passes through") instead of formula language ("connectives applied"). The point Enderton is making by choosing this example is that *this same recursive-definition pattern is what a real cost model looks like* — and it composes by max, not by sum, because gates on independent branches of an AND/OR run in parallel; only genuinely sequential dependency (one gate's output feeding another) adds to the critical path.
